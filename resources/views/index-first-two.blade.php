...
<body style="background-image: url({{ asset('img/background.jpg') }});">
    <div class="container">
        ...
        <section class="card-patient">
            <form class="card-patient__form" method="POST" action="{{ route('answers-first-two') }}" id="form">
                @csrf
...
<script>
    ...
    document.getElementById('submitButton').addEventListener('click', function() {
        ...
        //         selectedValuesAfter.before1 = before1Radios[i].parentNode.textContent.trim();
        //         break;
        //     }
        // }
        ...
        for (let i = 0; i < after1Radios.length; i++) {
            if (after1Radios[i].checked) {
                selectedValuesAfter.after1 = after1Radios[i].parentNode.textContent.trim();
                break;
            }
        }


        // selectedValuesAfter.before2 = [];
        // for (let i = 0; i < before2Radios.length; i++) {
        //     if (before2Radios[i].checked) {
        //         selectedValuesAfter.before2.push(before2Radios[i].parentNode.textContent.trim());
        //     }
        // }
        selectedValuesAfter.after2 = [];
        for (let i = 0; i < after2Radios.length; i++) {
            if (after2Radios[i].checked) {
                selectedValuesAfter.after2.push(after2Radios[i].parentNode.textContent.trim());
            }
        }
        // selectedValuesAfter.before3 = []; // Очищаем массив перед запуском цикла
        // for (let i = 0; i < before3Radios.length; i++) {
        //     if (before3Radios[i].checked) {
        //         selectedValuesAfter.before3.push(before3Radios[i].parentNode.textContent.trim());
        //     }
        // }

        selectedValuesAfter.after3 = []; // Очищаем массив перед запуском цикла
        for (let i = 0; i < after3Radios.length; i++) {
            if (after3Radios[i].checked) {
                selectedValuesAfter.after3.push(after3Radios[i].parentNode.textContent.trim());
            }
        }
        // selectedValuesAfter.before4 = []; // Очищаем массив перед запуском цикла
        // for (let i = 0; i < before4Radios.length; i++) {
        //     if (before4Radios[i].checked) {
        //         selectedValuesAfter.before4.push(before4Radios[i].parentNode.textContent.trim());
        //     }
        // }

        selectedValuesAfter.after4 = []; // Очищаем массив перед запуском цикла
        for (let i = 0; i < after4Radios.length; i++) {
            if (after4Radios[i].checked) {
                selectedValuesAfter.after4.push(after4Radios[i].parentNode.textContent.trim());
            }
        }
        // Добавляем значения selectedValuesAfter в FormData
        formData.append('selectedValuesAfter', JSON.stringify(selectedValuesAfter));
